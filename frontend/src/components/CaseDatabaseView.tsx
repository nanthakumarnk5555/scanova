    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 sm:p-8 rounded-3xl bg-white border border-slate-200 shadow-sm relative overflow-hidden">
        <div className="space-y-1.5">
          <div className="flex items-center space-x-2">
            <span className="px-3 py-1 rounded-full text-[10px] font-mono font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 flex items-center space-x-1">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            type="button"
            onClick={loadCases}
            className="flex items-center space-x-2 px-4 py-2.5 rounded-full bg-slate-50 border border-slate-200 hover:bg-slate-100 text-xs font-mono font-bold text-slate-700 transition-colors shadow-sm cursor-pointer"
          >
      ...
      {showSqlViewer && (
        <div className="p-5 rounded-3xl bg-slate-900 border border-slate-800 space-y-2.5 font-mono text-xs animate-in fade-in duration-200 text-white">
          ...
          <pre className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 text-[11px] text-amber-300 overflow-x-auto select-all">
            {currentSqlQuery}
          </pre>
        </div>
      )}
      ...
      <div className="p-5 rounded-3xl bg-white border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-3">
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            className="w-full pl-10 pr-3 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-emerald-500 font-mono"
          />
        </div>
            ...
            {(['all', 'Normal', 'Pneumonia'] as const).map((fn) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl capitalize transition-all cursor-pointer ${
                  filterFinding === fn ? 'bg-white text-slate-900 font-extrabold shadow-sm border border-slate-200' : 'text-slate-500 hover:text-slate-900'
                }`}
              >
          ...
          <div className="flex items-center space-x-1 bg-slate-50 p-1 rounded-2xl border border-slate-200 text-xs font-bold">
            {(['all', 'Concordant', 'Discordant', 'Pending'] as const).map((ag) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl capitalize transition-all cursor-pointer ${
                  filterAgreement === ag ? 'bg-white text-slate-900 font-extrabold shadow-sm border border-slate-200' : 'text-slate-500 hover:text-slate-900'
                }`}
              >
                {ag}
              </button>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <div className="flex items-center justify-end space-x-1.5">
                        <button
                          ...
                          className="px-3 py-1 rounded-full bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 text-[10px] cursor-pointer shadow-sm"
                        >
                          Inspect
                        </button>
                        <button
                          ...
                          className="px-3 py-1 rounded-full bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 text-[10px] font-bold transition-colors cursor-pointer shadow-sm"
                        >
                          PDF
                        </button>
                      </div>
          ...
          <div className="w-full max-w-xl bg-white border border-slate-200 rounded-3xl shadow-xl p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              ...
              <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 space-y-1">
                ...
                <p className="text-emerald-700">Latency: {selectedCase.prediction?.latency_ms?.toFixed(1) || '—'} ms</p>
              </div>
            </div>
            ...
            <div className="flex justify-end space-x-2 pt-2">
              ...
              <button
                ...
                className="px-5 py-2 rounded-full text-xs font-black cursor-pointer font-display bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm"
              >
                Download PDF
              </button>
            </div>
