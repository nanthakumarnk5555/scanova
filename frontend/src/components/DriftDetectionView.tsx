...
export const DriftDetectionView: React.FC = () => {
  ...
  if (loading && !driftData) {
    return (
      <div className="flex flex-col items-center justify-center h-96 space-y-3">
        ...
        <p className="text-sm font-semibold text-slate-700 font-display">Computing Population Stability Index (PSI)...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      {evalMessage && (
        <div className="p-3.5 rounded-2xl bg-blue-50 border border-blue-200 text-blue-800 text-xs flex items-center space-x-2 shadow-sm">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="p-6 rounded-3xl bg-white border border-slate-200/90 shadow-sm space-y-2">
          <div className="flex items-center justify-between text-slate-500">
            <span className="text-xs font-bold uppercase tracking-wider">Population Stability Index</span>
            <Activity className="w-4 h-4 text-blue-600" />
          </div>
          <p className={`text-3xl font-black font-display font-mono ${isSevere ? 'text-rose-600' : isModerate ? 'text-amber-600' : 'text-slate-900'}`}>
            {event?.psi_score !== undefined ? event.psi_score.toFixed(3) : '0.042'}
          </p>
          <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1 border-t border-slate-100">
            <span>Threshold limit:</span>
            <span className="font-bold text-slate-900 font-mono">&lt; 0.20 PSI</span>
          </div>
        </div>
        ...
        <div className="p-6 rounded-3xl bg-white border border-slate-200/90 shadow-sm space-y-2">
          <div className="flex items-center justify-between text-slate-500">
            <span className="text-xs font-bold uppercase tracking-wider">Drift Classification</span>
        ...
        <div className="p-6 rounded-3xl bg-white border border-slate-200/90 shadow-sm space-y-2">
          <div className="flex items-center justify-between text-slate-500">
            <span className="text-xs font-bold uppercase tracking-wider">KS 2-Sample Test</span>
            <BarChart3 className="w-4 h-4 text-indigo-600" />
          </div>
          <p className="text-3xl font-black text-slate-900 font-display font-mono">
            {event?.ks_statistic !== undefined ? event.ks_statistic.toFixed(3) : '0.120'}
          </p>
          <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1 border-t border-slate-100">
            <span>p-value:</span>
            <span className="font-bold text-indigo-700 font-mono">
              {event?.ks_p_value !== undefined ? event.ks_p_value.toFixed(3) : '0.450'}
            </span>
          </div>
        </div>
        ...
        <div className="p-6 rounded-3xl bg-white border border-slate-200/90 shadow-sm space-y-2">
          <div className="flex items-center justify-between text-slate-500">
            <span className="text-xs font-bold uppercase tracking-wider">KL Divergence (D_KL)</span>
            <TrendingDown className="w-4 h-4 text-teal-600" />
          </div>
          <p className="text-3xl font-black text-teal-700 font-display font-mono">
            {event?.kl_divergence !== undefined ? event.kl_divergence.toFixed(3) : '0.052'}
          </p>
          <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1 border-t border-slate-100">
        ...
        <div className="h-72 w-full pt-2">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={driftData?.histogram_comparison || []} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              />
              <Bar dataKey="baseline_freq" fill="#CBD5E1" name="Baseline Validation (%)" radius={[6, 6, 0, 0]} />
              <Bar dataKey="current_freq" fill="#2563EB" name="Current Ingestion (%)" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="p-6 rounded-3xl bg-white border border-slate-200/90 shadow-sm space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80">
            <span className="text-xs text-slate-500 font-medium">Baseline Sample Size:</span>
            <p className="text-lg font-extrabold text-slate-900 mt-0.5 font-display font-mono">
              {event?.summary?.baseline_sample_size || 200} cases
            </p>
          </div>
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80">
            <span className="text-xs text-slate-500 font-medium">Active Ingestion Sample:</span>
            <p className="text-lg font-extrabold text-slate-900 mt-0.5 font-display font-mono">
              {event?.summary?.current_sample_size || 45} cases
            </p>
          </div>
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80">
            <span className="text-xs text-slate-500 font-medium">Mean Confidence Shift:</span>
            <p className="text-lg font-extrabold text-blue-700 mt-0.5 font-display font-mono">
              {event?.summary?.mean_shift !== undefined ? `${(event.summary.mean_shift * 100).toFixed(2)}%` : '+1.4%'}
            </p>
          </div>
        </div>

        <p className="text-xs text-slate-600 p-4 rounded-2xl bg-blue-50/60 border border-blue-100 font-sans leading-relaxed">
          <span className="font-bold text-blue-800">Automated Guardrail Note:</span>{' '}
