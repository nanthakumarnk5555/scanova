...
def build_pdf_report(filename="SCANOVA_Final_Year_Project_Report.pdf"):
    print("Initializing SCANOVA 100-page Project Report Document Generator...")

    # Standard Anna University / Engineering margins
    ...
    # Define custom typography matching Anna University style
    ...
    t_stud = make_table(student_data, col_widths=[220, 180])
    story.append(t_stud)
    ...
    # 4. Table of Contents (Pages iv to x) - 7 pages exactly matching sample format
    toc_pages = [
        # TOC Page 1 (Page iv)
        [
            ("ABSTRACT", "iii", True),
    ...
    for toc_idx, toc_page_entries in enumerate(toc_pages):
    ...
    # 7. List of Symbols & Abbreviations (Page xiii)
    ...
    story.append(p("PyTorch and Torchvision power the dual deep learning inference engines. DenseNet-121 utilizes dense connectivity layers for maximum pulmonary feature reuse, while ResNet-50 leverages residual bottleneck blocks for bone fracture detection. Grad-CAM computes gradients at the final convolutional layers to generate spatial heatmaps."))
    ...
    story.append(p("The stack combines developer productivity, high throughput, mathematical rigor, and enterprise security, establishing a robust foundation for hospital clinical AI operations."))
    story.append(PageBreak())
    ...
    story.extend(fig_img("fig_7_1_system_arch.png", "Figure 7.1: Detailed Multi-Tier System Architecture of SCANOVA", w=440, h=215))
    story.append(Spacer(1, 8))
    ...
    story.extend(fig_img("fig_7_2_component_arch.png", "Figure 7.2: Component-Level Interaction Flowchart of SCANOVA", w=440, h=205))
    story.append(Spacer(1, 8))
    ...
    story.append(p("The system architecture guarantees complete isolation between diagnostic models. The CheXNet DenseNet-121 and Trauma ResNet-50 models run in dedicated execution contexts, preventing weight contamination and allowing independent scaling."))
    ...
    story.append(b("6. Grad-CAM backpropagates gradients through the final conv layer to produce an activation map."))
    story.append(b("7. Diagnostic findings and heatmap are displayed on the Radiologist Diagnostic Studio."))
    story.append(b("8. Radiologist reviews findings, submits adjudication verdict (Agree/Override), and adds clinical notes."))
    story.append(b("9. Population Stability Index (PSI) engine updates rolling surveillance distributions."))
    story.append(b("10. Urgent SLA alerts are dispatched if high-risk pathologies exceed emergency thresholds."))
    story.append(Spacer(1, 10))

    story.extend(fig_img("fig_7_3_workflow.png", "Figure 7.3: End-to-End Diagnostic Surveillance System Workflow", w=440, h=215))
    story.append(Spacer(1, 8))
    ...
    story.extend(fig_img("fig_7_4_dfd.png", "Figure 7.4: Data Flow Diagram (DFD Level 1) of Diagnostic Pipeline", w=440, h=205))
    story.append(Spacer(1, 8))
    ...
    story.extend(fig_img("fig_7_5_usecase.png", "Figure 7.5: Use Case Diagram for Radiologists and System Administrators", w=440, h=205))
    story.append(Spacer(1, 8))
    ...
    story.append(Spacer(1, 8))
    story.append(p("Figure 8.1 illustrates the Entity Relationship Diagram (ERD) of SCANOVA, demonstrating the foreign key links connecting user accounts, diagnostic studies, neural predictions, adjudications, and surveillance records."))
    ...
    story.append(h1("8.12 Database Design Summary"))
    story.append(p("The database architecture provides high-performance structured storage, reliable foreign key integrity, and complete audit readiness for hospital radiology departments."))
    story.append(PageBreak())
    ...
    story.append(h1("9.11 Module N10: Radiologist Adjudication & Peer Review"))
    story.append(p("Module N10 provides the human-in-the-loop validation console. Radiologists record their diagnostic agreement or override decisions, logging qualitative observations and updating department-wide Fleiss' kappa concordance metrics."))

    story.append(h1("9.12 Module N11: Population Stability Index (PSI) Drift Surveillance"))
    story.append(p("Module N11 calculates the Population Stability Index across 10 probability bins comparing live production inference batches against validated baseline cohorts:"))
    ...
    story.append(h1("9.13 Module N12: Urgent Clinical Alerts & SLA Incident Manager"))
    story.append(p("Module N12 prioritizes studies exhibiting critical diagnostic findings (confidence > 80%), assigning emergency 60-minute triage SLAs and broadcasting real-time notification alerts across the clinical workstation network."))

    story.append(h1("9.14 Module Integration"))
    story.append(p("All twelve modules integrate seamlessly through asynchronous REST APIs, forming a unified, self-monitoring diagnostic surveillance ecosystem."))
    story.append(PageBreak())
    ...
    story.append(h1("10.2 Frontend Implementation"))
    story.append(p("The frontend is built using React 18, TypeScript, Vite, and Tailwind CSS. The interface is organized into five primary functional views: (1) Executive Surveillance Dashboard with real-time KPI cards; (2) AI Diagnostic Studio with drag-and-drop file ingestion and dual-canvas Grad-CAM viewers; (3) Population Drift Surveillance Console with interactive PSI distribution charts; (4) Radiologist Adjudication Console for recording concordance; and (5) Clinical Alerts Notification Center."))

    story.append(h1("10.3 Backend & Database Implementation"))
    story.append(p("The backend is implemented in Python using the FastAPI framework running on Uvicorn ASGI servers. FastAPI's async request handlers process radiograph uploads concurrently, dispatching tensor operations to PyTorch while managing database read/write transactions through SQLAlchemy ORM."))

    story.append(h1("10.4 Image Ingestion & Modality Guardrail Pipeline"))
    story.append(p("Upon file upload, the ingestion pipeline reads raw byte buffers, performs DICOM tag stripping, and evaluates image characteristics. The Modality Guardrail checks whether the radiograph matches the selected diagnostic modality (Chest CXR vs Skeletal series), immediately rejecting invalid inputs before GPU memory allocation."))

    story.append(h1("10.5 Dual Deep Learning Inference & Grad-CAM Pipeline"))
    ...
    story.append(p("The end-to-end working principle integrates ingestion, guardrail validation, neural inference, visual explainability, statistical surveillance, and clinical feedback into an unbroken closed-loop diagnostic ecosystem."))
    story.append(Spacer(1, 10))

    story.extend(fig_img("fig_10_1_processing_flow.png", "Figure 10.1: Clinical Diagnostic & Surveillance Processing Workflow", w=440, h=215))
    story.append(Spacer(1, 8))
    ...
    story.append(p("The workflow ensures rapid turn-around (< 300 ms) while guaranteeing that every AI decision is accompanied by spatial explainability and monitored for statistical drift."))
    ...
    story.append(Spacer(1, 10))
    story.append(Paragraph("<b>Live Production Deployment URL:</b>", ParagraphStyle('URLH', fontName='Times-Bold', fontSize=12, alignment=TA_CENTER)))
    story.append(Spacer(1, 4))
    story.append(Paragraph("<u><font color='#0284c7'>https://scanova-navy.vercel.app/</font></u>", ParagraphStyle('URLL', fontName='Times-Bold', fontSize=13, alignment=TA_CENTER)))
    story.append(Spacer(1, 15))
    ...
    story.extend(fig_img("fig_14_1_qrcode.png", "Figure 14.1: SCANOVA Live Web Application Deployment QR Code", w=160, h=160))
    story.append(Spacer(1, 10))
    ...
    # Screen 15.4: Pneumonia & Grad-CAM
    ...
    # Screen 15.6: Drift Surveillance
    ...
    story.append(h1("16.5 Grad-CAM Saliency Map Generator"))
    ...
    story.append(h1("16.6 Population Stability Index (PSI) Drift Calculation"))
    ...
    story.append(h1("16.7 Note on Source Code & Academic Integrity"))
